...
findStringsLongerThanTenChars2 l = [str | str <- l, length str > 10]
...
buildPairsStringLength = map (\str -> (str, length str))

buildPairsStringLength2 :: [String] -> [(String, Int)]
buildPairsStringLength2 l = [(str, length str) | str <- l]
...
newtonRaphsonSolve x_old atol g dg =
  let approximations = newtonRaphson x_old g dg
      differences = zipWith (\x_new x_old -> abs (x_new - x_old)) (tail approximations) approximations
   in fst $ head $ dropWhile (\(_, diff) -> diff > atol) (zip approximations differences)
...
babylonianMethod a x_old atol =
    let approximations = iterate (\x -> 0.5 * (x + a / x)) x_old
        differences = zipWith (\x_new x_old -> abs (x_new - x_old)) (tail approximations) approximations
    in fst $ head $ dropWhile (\(_, diff) -> diff > atol) (zip approximations differences)
