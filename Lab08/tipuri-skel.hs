...
insertElem root@(BSTNode val lf rg) elem
  | elem == val = root
  | elem < val = BSTNode val (insertElem lf elem) rg
  | elem > val = BSTNode val lf (insertElem rg elem)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem root@(BSTNode val lf rg) elem
  | elem == val = Just val
  | elem < val = findElem lf elem
  | elem > val = findElem rg elem

inorder :: BST a -> [a]
inorder BSTNil = []
inorder root@(BSTNode val lf rg) = inorder lf ++ val : inorder rg

size :: BST a -> Int
size BSTNil = 0
size root@(BSTNode val lf rg) = 1 + size rg + size lf

height :: BST a -> Int
height BSTNil = 0
height root@(BSTNode val lf rg) = 1 + max (height lf) (height rg)

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced root@(BSTNode val lf rg)
    | not (isBalanced lf) = False
    | not (isBalanced rg) = False
    | abs (height lf - height rg) > 1 = False
    | otherwise = True
...
consElem a (Atom b) = List [Atom a, Atom b]
consElem a (List xs) = List $ Atom a : xs

consList :: NestedList a -> NestedList a -> NestedList a
consList as (Atom b) = List [as, Atom b]
consList as (List xs) = List $ as : xs

headList :: NestedList a -> NestedList a
headList (Atom a) = error "Undefined"
headList (List as) = head as

tailList :: NestedList a -> NestedList a
tailList (Atom a) = error "Undefined"
tailList (List as) = List $ tail as

deepEqual :: (Eq a) => NestedList a -> NestedList a -> Bool
deepEqual (Atom a) (Atom b) = a == b
deepEqual (List as) (List xs) = equalLen && and (zipWith deepEqual as xs) 
        where
                equalLen = length as == length xs
deepEqual _ _ = False

flatten :: NestedList a -> [a]
flatten (Atom a) = [a]
flatten (List as) = foldl (++) [] (map flatten as)
