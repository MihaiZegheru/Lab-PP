...
-- foldl
reverseList5 :: [a] -> [a]
reverseList5 = foldl (flip (:)) []

-- foldr
reverseList6 :: [a] -> [a]
reverseList6 = foldr (\x acc -> acc ++ [x]) []
...
numToBase4 n b = case (n, b) of
  (0, _) -> []
  (n, b) -> numToBase4 (n `quot` b) b ++ [n `mod` b]
...
removeDuplicatesLeft = foldl (\acc x -> if x `elem` acc then acc else acc ++ [x]) []
...
removeDuplicatesRight = foldr (\x acc -> if x `elem` acc then acc else x : acc) []
...
merge [] l = l
merge l [] = l
merge fullList1@(x1 : l1) fullList2@(x2 : l2) =
  if x1 <= x2
    then x1 : merge l1 fullList2
    else x2 : merge fullList1 l2

mergeSort :: [Int] -> [Int]
mergeSort lst =
  let mid lst = length lst `quot` 2
      first lst = take (mid lst) lst
      second lst = drop (mid lst) lst
   in if length lst <= 1
        then lst
        else merge (mergeSort (first lst)) (mergeSort (second lst))
