...
(define (stream-zip-with f s1 s2)
  (if (or (stream-empty? s1) (stream-empty? s2))
      empty-stream
      (stream-cons (f (stream-first s1) (stream-first s2)) (stream-zip-with f (stream-rest s1) (stream-rest s2)))))
...
(define leibniz-series-1 ;; explicit
   (let for ((a 1) (sign 1))
     (stream-cons (/ sign a) (for (+ a 2) (- sign)))))

(define leibniz-series-2 ;; implicit
   (stream-zip-with / (repeat 1 -1) (stream-filter odd? (naturals-from 1))))
...
(define (play x f1 f2)
  (letrec ((foo (stream-cons x (stream-zip-with apply
                                                (repeat f1 f2)
                                                (stream-map list foo)))))
    foo))
...
(define (winner n)
  (car (stream-first (stream-filter (lambda (round) (<= (cdr round) 0))
                                    (stream-zip-with cons
                                                     (repeat 2 1)
                                                     (play n
                                                           (lambda (x) (if (divisible? x 3) (- x 1) (- x 3)))
                                                           (lambda (x) (if (divisible? x 4) (- x 2) (- x 1)))))))))
...
(define (not-prime? n)
  (let loop ((divisors primes))
    (let ((div (stream-first divisors)))
      (and (<= (* div div) n)
           (or (divisible? n div) (loop (stream-rest divisors)))))))
...
(define (rotations n)
  (let* ((str (number->string n)) (len (string-length str)))
    (let loop ((str str) (len len))
      (if (zero? len)
          empty-stream
          (stream-cons (string->number str) (loop (string-append (substring str 1) (substring str 0 1)) (sub1 len)))))))
...
(define (circular-prime? n)
  (stream-empty? (stream-filter not-prime? (rotations n))))
