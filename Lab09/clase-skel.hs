...
instance (Eq a) => Eq (BST a) where
  (==) BSTNil BSTNil = True
  (==) (BSTNod val1 lf1 rg1) (BSTNod val2 lf2 rg2) = val1 == val2 && lf1 == lf2 && rg1 == rg2
  (==) _ _ = False
...
showLevel :: Show a => Int -> BST a -> [Char]
showLevel _ BSTNil = ""
showLevel h (BSTNod val lf rg) = replicate h '\t' ++ show val ++ "\n" ++ showLevel (h + 1) lf ++ showLevel (h + 1) rg

instance (Show a) => Show (BST a) where
  show BSTNil = ""
  show tree =  showLevel 0 tree
...
instance Invertible (BST a) where
  ...
  invert (BSTNod val lf rg) = BSTNod val (invert rg) (invert lf)
...
instance Functor BST where
  ...
  fmap f (BSTNod val lf rg) = BSTNod (f val) (fmap f lf) (fmap f rg)
...
instance Foldable BST where
  ...
  foldr f acc (BSTNod val lf rg) = foldr f (f val (foldr f acc rg )) lf
