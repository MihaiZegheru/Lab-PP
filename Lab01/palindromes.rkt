...
(define (num->base n b)
  (if (zero? n) `() (append (num->base (quotient n b) b) (list (remainder n b)))))
...
(define (deep-rev L)
  (cond
    ([null? L] `())
    ([list? L] (append (deep-rev (cdr L)) (list (deep-rev (car L)))))
    (else L)))
...
;; Abordarea recomandată este să descrieți CE este un palindrom,s
...
(define (all-palindromes? n Bases)
  (if (null? Bases)
      #t
      (and (all-palindromes? n (cdr Bases))
           (equal? (num->base n (car Bases)) (rev (num->base n (car Bases))))
           )
      )
  )
...
(define (palindromes-to-n n Bases)
  (if (zero? n)
      (if (all-palindromes? n Bases) (list n) `())
...
(define (count-pal-bases n Bases)
  (if (null? Bases)
      0
      (+ (count-pal-bases n (cdr Bases))
         (if (palindrome? (num->base n (car Bases))) 1 0))
      )
  )

(define (first-b-pal start b)
  (if (>= (count-pal-bases start `(2 3 4 5 6 7 8 9 10)) b)
      start
...
(define (longest-palindrome n)
  (if (palindrome? (num->base n 10))
       n
       (max (longest-palindrome (quotient n 10))
